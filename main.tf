...
locals {
  aws_key_name = local.ssh_key_content == "" ? var.aws_key_name : element(coalescelist(aws_key_pair.deployer.*.key_name, [""]), 0)
}
...
module "dcos-master-instances" {
  ...
  aws_security_group_ids = [
    module.dcos-security-groups.internal,
    module.dcos-security-groups.admin,
    module.dcos-security-groups.master_lb,
  ]
  aws_key_name         = local.aws_key_name
...
module "dcos-privateagent-instances" {
  ...
  aws_ami              = var.private_agents_aws_ami == "" ? var.aws_ami : var.private_agents_aws_ami
...
module "dcos-publicagent-instances" {
  ...
  aws_security_group_ids = [
    module.dcos-security-groups.internal,
    module.dcos-security-groups.admin,
    module.dcos-security-groups.public_agents,
  ]
  aws_key_name         = local.aws_key_name
  num_public_agents    = var.num_public_agents
  dcos_instance_os     = coalesce(var.public_agents_os, var.dcos_instance_os)
  aws_ami              = var.public_agents_aws_ami == "" ? var.aws_ami : var.public_agents_aws_ami
...
module "dcos-lb" {
  ...
  security_groups_masters = [
    module.dcos-security-groups.admin,
    module.dcos-security-groups.internal,
    module.dcos-security-groups.master_lb,
  ]
  security_groups_masters_internal = [module.dcos-security-groups.internal]
  security_groups_public_agents = [
    module.dcos-security-groups.internal,
    module.dcos-security-groups.admin,
  ]
